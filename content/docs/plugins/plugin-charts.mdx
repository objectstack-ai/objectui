...
| `position` | `'left' \| 'right' \| 'top' \| 'bottom'` | The side of the chart the axis belongs to. |
| `logarithmic` | `boolean` | Logarithmic scale on a numeric axis. |

⚠️ On `xAxis`, `min`, `max`, `stepSize` and `logarithmic` currently take effect only when the same object also carries `title`, `format` or `showGridLines`.
...
const schema: ChartSchema = {
  ...
  yAxis: [
    ...
    // the second entry is the right-hand axis the `margin` line binds to
    { field: 'margin', title: { en: 'Margin', fr: 'Marge' }, format: '0.0%' }
  ],
  series: [
    { name: 'revenue' },
    { name: 'margin', type: 'line', yAxis: 'right' }
  ]
}
```
