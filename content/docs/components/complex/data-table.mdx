...
What the flag does **not** cover: the table's client-side search and sort still run
over the raw values, so where they run on the client they can reveal whether a
masked value matches or how it orders.
...
keeps the flag. The same rule leaves masked fields out of `object-grid`'s own client
export (CSV and JSON) and draws them through their `cell` on its mobile card. Other
producers of `data-table` columns (the related list, `object-data-table`) do not set
the flag yet.
