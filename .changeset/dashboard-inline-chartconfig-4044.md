...
Three keys are deliberately still not forwarded. `type` is refused because the widget's own `type` picks the chart family on this path. `xAxis` / `yAxis` / `series` are refused because whether an authored axis beats the dataset-derived one is an open protocol question, filed for the spec seat as objectstack-ai/objectstack#17385. `aria` is refused because nothing on this path reads it — the chart's one accessible name comes from `description`.
