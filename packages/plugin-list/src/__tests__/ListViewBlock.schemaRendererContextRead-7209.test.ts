...
/**
 * objectui#7209 — `ListViewBlock` reads `SchemaRendererContext` AS DECLARED.
 *
 * It used to read it through
 * `useContext(SchemaRendererContext as React.Context<any>)`, which erased
 ...
 * `ctx.formValues ?? ctx.data` channel.
 *
...
const SITE_FILE = slash(join(HERE, '..', 'ListViewBlock.tsx'));
const CONTEXT_FILE = slash(join(REPO_ROOT, 'packages', 'react', 'src', 'context', 'SchemaRendererContext.tsx'));

/** The registry block that reads the context, by the name it is declared under. */
const SITES = ['ListViewBlock'] as const;
