...
California" pair). The clear is saved like any other edit: a single select (or
radio) is emptied to `null`, which a save writes as "clear the stored value", and
a multiple select keeps only the values still offered.
