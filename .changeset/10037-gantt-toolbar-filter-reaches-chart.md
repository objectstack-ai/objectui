...
Both controls were offered on a gantt view and changed nothing the user could see. Every
other view draws the rows `ListView` fetched, and that fetch applies the authored filter,
the toolbar's filter group and the chips together. The gantt chart is the one view that
queries for itself — its registered renderer takes only the node's `schema` — and the
node it was handed carried the authored `filter` alone.
