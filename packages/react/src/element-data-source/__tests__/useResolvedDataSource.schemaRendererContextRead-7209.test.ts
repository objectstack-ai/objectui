...
/**
 * objectui#7209 — `useResolvedDataSource` reads `SchemaRendererContext` AS
 * DECLARED.
 *
 * It used to read it through
 * `React.useContext(SchemaRendererContext as React.Context<any>)`, which erased
 * `SchemaRendererContextType` at the read: a member the context does not
 * declare type-checked clean — the mechanism behind objectui#7206's phantom
 * `ctx.formValues ?? ctx.data` channel. The hook's published signature is
 * unchanged; its only remaining assertion is the caller's `T` on the way out.
 *
...
const SITE_FILE = slash(join(HERE, '..', 'ElementDataSourceGate.tsx'));
...
const SITES = ['useResolvedDataSource'] as const;
