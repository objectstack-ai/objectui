...
Russian and Arabic now read correctly at every count on the tab-count badge's
accessible name, the activity-feed reaction chip and the comment thread's count
and reaction tooltip (objectui#10242).
...
three noun forms for whole numbers, or Arabic, which needs more. The plain key held
a single noun form, so it was wrong at some of the counts it serves. A Russian tab
