...
- A section's **own settings apply whether or not it has a heading.** Its
  `visibleWhen` hides the whole group, fields included, on every layout. A
  section with no `label` is never un-gated.
...
The modal's `contentLayout: 'tabbed'` draws tabs instead of rows, and tabs do
not collapse.
