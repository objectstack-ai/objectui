const ru = {
  ...
  console: {
    ...
    readRate: {
      ratioTitle: 'Необычный объём чтения в этой среде',
  ...
  profile: {
    ...
    language: {
      title: "Язык",
      description: "Язык уведомлений и сообщений, которые отправляются вам. Язык интерфейса выбирается отдельно — в меню с глобусом.",
      label: "Предпочитаемый язык",
      systemDefault: "Использовать значение по умолчанию для развёртывания",
      save: "Сохранить",
      saved: "Языковые настройки обновлены.",
      readOnly: "Язык вашей учётной записи настраивает администратор.",
    },
    password: {
  ...
  empty: {
    ...
    appNotAvailableDescription: "Это приложение недоступно — повторите попытку через мгновение.",
    appNotFound: "Не удалось открыть это приложение",
    appNotFoundDescription: "Сервер не вернул это приложение для вашей учётной записи.",
    appUnreachable: "Не удалось связаться с сервером",
    appUnreachableDescription: "Не удалось проверить это приложение. Повторите попытку через мгновение.",
