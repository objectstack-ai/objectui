...
```plaintext
{
  ...
  search?: string,                  // full-text term, sent as $search — applies on EVERY provider
  searchableFields?: string[],      // narrows what `search` matches, sent as $searchFields with a term
}
```
...
`search` is the chart's full-text term. A list view's toolbar Search box writes
it (with the view's `searchableFields`) onto its gantt node, because the chart
runs its own query; the server decides which fields a term matches unless
`searchableFields` narrows them, and a `searchableFields` with no term sends
nothing.
