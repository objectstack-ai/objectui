...
`type` and the object's field type, so a view that shows a `secret` field as `text`
keeps the flag. The same rule leaves every masked field of the grid's object out of
`object-grid`'s own client export (CSV and JSON), draws those fields through their
`cell` on its mobile card, and refuses them as grouping keys (the entry is ignored
with a console warning). What `object-grid` does not cover: the server-streamed
export (`exportDownload`) sends the masked columns as before and relies on the
server's masking, and the client JSON export writes an expanded lookup record whole,
so a credential field of the related object is not pruned. Other producers of
