...
keeps the flag. Once the object schema has loaded, the same rule leaves every masked
...
schema, which the grid fetches after first paint, so until it settles an untyped
view column over a `password` / `secret` field draws and hands out the raw value
(objectui#NEWCARD). The server-streamed export (`exportDownload`) sends the masked
