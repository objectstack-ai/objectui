 ...
 *
 * This file REPLACES `ObjectCalendar.overlayShellCollapse-9113.test.tsx`, which pinned what
 * was true on the base tree: `ObjectCalendar` rendered the one
...
describe('objectui#9299 — `ObjectCalendar` honours all four overlay modes through the shared shell', () => {
  ...
  it('item 4 — the shell is handed both width keys, so an existing width carries over', async () => {
    const r = await readingsFor('drawer');
    expect(r.boundary.storageKey).toBe('drawer-width:duly_event');
    expect(r.boundary.legacyStorageKey).toBe('objectui.drawerWidth.duly_event');
  });
});
