...
/**
 * objectui#10572 — an `object-calendar` block re-reads its events when the data-invalidation
 * bus (`notifyDataChanged` from `@object-ui/react`) reports a write to the
 * object it queries.
 *
...
function makeDataSource() {
  return {
    find: vi.fn(async () => ({ data: [{ id: '1', name: 'Kickoff', due: '2026-09-01' }], total: 1 })),
    findOne: vi.fn(),
    create: vi.fn(),
    update: vi.fn(),
    delete: vi.fn(),
    getObjectSchema: vi.fn(async () => ({ name: 'event', fields: { name: { type: 'text' }, due: { type: 'date' } } })),
  };
}
...
const BLOCK = { type: 'object-calendar', objectName: 'event', dateField: 'due', titleField: 'name' };

describe('object-calendar re-reads on the data-invalidation bus (objectui#10572)', () => {
  ...
  it('a change to its own object re-runs its query once; another object does not', async () => {
    ...
    await act(async () => {
      notifyDataChanged({ objectName: 'event', recordId: '1' });
    });
    await settle();
