...
/**
 * objectui#10572 — an `object-kanban` block re-reads its cards when the data-invalidation
...
// the chunk at module scope keeps the cold transform out of the settle window
// (the objectui#3010 rule, same specifier as `index.tsx`'s factory).
import './KanbanImpl';
...
function makeDataSource() {
  return {
    find: vi.fn(async () => ({ data: [{ id: '1', name: 'Acme', status: 'open' }], total: 1 })),
    findOne: vi.fn(),
    create: vi.fn(),
    update: vi.fn(),
    delete: vi.fn(),
...
describe('object-kanban re-reads on the data-invalidation bus (objectui#10572)', () => {
  ...
  it('a block drawing inline rows queries nothing on an invalidation', async () => {
    const ds = makeDataSource();
    renderBlock({ ...BLOCK, data: [{ id: '1', name: 'Inline', status: 'open' }] }, ds);
    await settle();
