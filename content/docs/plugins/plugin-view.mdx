...
(`ObjectView.tsx`'s `if (!dataSource || !schema.objectName) return;` guards), so
the result is a silent empty view rather than an error.
...
`ObjectView.tsx` — the non-grid data fetch, the `gridSchema` memo, and the
`mergedFilters` / `mergedSort` pair handed to a host-supplied list renderer —
so the keys below behave the same on every rendering path.
...
> tombstone), and `ObjectView` no longer reads it on any of the three
> paths above (nor does `ObjectGrid`): a view that still carries it renders
> **unsorted**. Rename the key to `sort` and wrap the value in an array —
...
key and the `@deprecated` legacy spelling it replaced. Both work; write the
canonical one. (A fourth pair, `sort` / `defaultSort`, used to be listed here;
its legacy half is retired — see the note above.)
...
the right are simply no longer the ones to reach for.
...
`sort` has no legacy tier since `table.defaultSort` was retired). If you never
write `listViews`, that first tier never applies.
...
`type` as a back-compat fallback (`ComponentRegistry.register` in
`packages/core/src/registry/Registry.ts`, its `!meta?.skipFallback` branch). No
call in this package sets `skipFallback`. Note the
...
| `NamedListView` | one entry of `listViews` — toolbar toggles go under `userActions` (`search`, `sort`, `filter`, `group`, `hideFields`, `rowHeight`, `rowColor`) and `showDescription` under `appearance`; the legacy bare `show*` spellings are `never` on this type (objectui#7924), though stored views carrying them still load |
